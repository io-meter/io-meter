...
![Empty Stack](/img/immutable/immutable-stack-head-0.png)

接下来我们向栈中压入几个元素，得到的栈的状态如下:

![Push](/img/immutable/immutable-stack-head-1.png)

那么出栈操作怎么实现呢？我们把指向栈顶的头向后移动几个元素就可以了:

![Pop](/img/immutable/immutable-stack-head-2.png)

从新的位置开始插入其它的元素呢？其实只是在栈中开启一个新的分支:

![Push](/img/immutable/immutable-stack-head-3.png)
...
![Tree Structure](/img/immutable/tree-structure.png)
...
![Immutable Tree](/img/immutable/immutable-tree.png)
...
![Git Object Model](/img/immutable/git-object-model.png)
...
![Lock-free Data Structure](/img/immutable/lock-free-data-structure-initial.png)
...
![Lock-free Data Structure: Reference Transition](/img/immutable/lock-free-data-structure-transit.png)

由于对于引用的操作是原子的，因此在原子操作成功之前，任意线程发起的读请求仍然获得的是老的内容，
而一旦这一操作在瞬间完成，所有新的访问都将获得新的内容。不会有任何一个线程会获得中间状态，
这也就保证了多线程环境下的数据安全。

![Lock-free Data Structure: Tranistion Finished](/img/immutable/lock-free-data-structure-finished.png)
...
![Log-Structured Merge Tree](/img/immutable/log-structure-merge-tree.png)

可以看到 LSMT 把所有的数据组织成大小接近的文件块，每个文件块内的记录通过指针引用形成树状的结构，
...
![Log-Structured Merge Tree: Append Modification](/img/immutable/log-structure-merge-tree-append-path.png)

如上图所示，借用 Immutable Tree 的实现思想，当操作执行时，我们会生成一条新的修改路径，
而原来的节点仍然在原来的位置不会被修改。之后我们所做的只是将新产生的节点一个接一个追加到文件的结尾。
追加结束之后的状态如下图所示:

![Log-Structured Merge Tree: Append Finished](/img/immutable/log-structure-merge-tree-append-finish.png)
...
![Log-Structured Merge Tree: Change Root](/img/immutable/log-structure-merge-tree-change-root.png)
...
![Log-Structured Merge Tree: Commited](/img/immutable/log-structure-merge-tree-change-root-finish.png)

可以看到，新的修改提交成功之后，老的一些记录就变成失效的了，为了清理这些失效的记录，
LSMT 还设计在后台执行清理操作，清理的操作逻辑也很简单:

![Log-Structured Merge Tree: Clean](/img/immutable/log-structure-merge-tree-block-clean.png)
